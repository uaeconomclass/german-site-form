...
## Статус імплементації (важливо)

Форма вже реалізована як **data-driven wizard** на основі JSON-спеки:

- кроки/поля/валідації/опції: `src/energieausweis-form/spec/`
- рантайм-рендерер: `src/energieausweis-form/runtime/*.js`

Цей `FORM-SPEC.md` лишається **людським/текстовим описом**, витягнутим із DOCX.
Якщо тут є розбіжності з реальним UI, **джерелом істини вважаємо JSON-спеку** (бо саме вона збирається в `preview/`/`docs/`).

---

## Загальний вигляд форми

![Загальний вигляд форми](assets/images/overview/form-overview.jpg)
...
> Вимоги відрізняються залежно від типу паспорту (Bedarfsausweis vs Verbrauchsausweis).
> Повні тексти юридичних обґрунтувань — в `TOOLTIPS-DE.md`, секція 6.

### Upload — Verbrauchsausweis (РАНІШЕ ВІДСУТНЄ!)

> Ці поля потрібні ТІЛЬКИ якщо Ausweisart = Verbrauchsausweis.

| Документ | Обов'язкове | Примітка |
|----------|-------------|----------|
| Heizkostenabrechnungen (рахунки за опалення) | **Так** | За останні 3 роки |
| Verbrauchsdaten (дані споживання) | **Так** | Фактичне споживання енергії |
...
## Повні тексти підказок

Всі tooltip/info-popup тексти німецькою (дослівно з DOCX) зібрані в окремому файлі:
- **`TOOLTIPS-DE.md`** — готові до використання в UI

Файл містить повні описи для:
1. **Außenwand** — 8 типів стін з Erkennungsmerkmale (матеріал, роки, колір, вага, свердління, пил, енергетична оцінка)
2. **Fenster** — 6 типів вікон з U-значеннями, роками, матеріалами рами
3. **Heizung** — 13 типів опалення з температурними режимами та описами
4. **Warmwasser** — 7 типів ГВП з описами
5. **Lüftung** — 5 типів вентиляції з описами
6. **Upload-тексти** — юридичні обґрунтування GEG для кожного upload-блоку
7. **Rechtssichere Hinweise** — всі юридичні disclaimers та визначення (Wohnfläche, Nutzfläche, тощо)
